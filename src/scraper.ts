...
export function isStaleResult(result: JobResult): boolean {
  return result.status === 'success' && (result.companyMismatch || result.lateOverlayDetected);
}
...
export async function clearBlockingOverlays(
  page: Page,
  { timeoutMs = 15000, pollIntervalMs = 250, requiredConsecutiveClear = 4 }: OverlayClearOptions = {}
): Promise<boolean> {
  ...
  while (Date.now() < deadline) {
    ...
    const button = overlay.getByRole('button', { name: /reject|dismiss|accept/i }).first();
    try {
...
async function collectJobIds(page: Page): Promise<Set<string>> {
  const ids = await page.evaluate(() => {
    ...
    const lis = Array.from(g.document.querySelectorAll('ul.jobs-search__results-list > li')).filter((li) =>
      li.querySelector('h3')
    );
    return lis.map((li, i) => {
      const urn = li.querySelector('.base-card')?.getAttribute('data-entity-urn') ?? '';
...
export async function scrollLoadPhase(
  ...
  const { maxScrollAttempts = 60, stableScrollsToStop = 3, onProgress } = options;
...
// the DOM.
async function pollForNewJobs(page: Page, previousCount: number): Promise<number> {
...
export async function clickLoadPhase(
  ...
  const { maxSeeMoreClicks = 200, stableClicksToStop = 3, clickRetryAttempts, onProgress } = options;
...
async function clickWithOverlayRetries(locator: Locator, page: Page, maxAttempts = 4): Promise<void> {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    await clearBlockingOverlays(page, { timeoutMs: 4000, requiredConsecutiveClear: 2, pollIntervalMs: 200 });
    try {
...
// the "See more" button clickWithOverlayRetries() already guards), so after
// the click, give the overlay one more dedicated clear pass and only fail
// the job if it's still genuinely stuck behind it.
async function dismissOverlayAfterClick(page: Page): Promise<void> {
  ...
  if (stillBlocked) {
    throw new Error('Blocked by LinkedIn sign-in wall (could not dismiss dialog)');
  }
}
...
export async function scrapeJob(
  ...
  // Human-readable stand-in title used when the real title can't be read;
  ...
  try {
    ...
    if ((await jobItem.locator('h3').first().count()) === 0) {
      return {
        index,
        title: null,
        company: null,
...
export async function scrapeAllJobsOnce(ctx: ScrapeContext, results: JobResult[]): Promise<number[]> {
...
async function retryStaleJobs(ctx: ScrapeContext, results: JobResult[], staleIndices: number[]): Promise<void> {
  const delayBetweenJobsMs = ctx.delayBetweenJobsMs ?? 700;
...
export const runScrape: RunScraper = async ({ onProgress, searchParams, scraperOptions }: RunScrapeOptions) => {
  ...
  try {
    ...
    await clearBlockingOverlays(page, {
      ...
      requiredConsecutiveClear: scraperOptions?.overlayClear?.requiredConsecutiveClear ?? 5,
