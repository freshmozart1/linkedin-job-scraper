...
import { createFakeBrowser, createFakeContext, createFakePage } from './helpers/fakePlaywright';
...
function makeLookup(options: {
  ...
  const promise = createCompanyLookup(createFakeBrowser({ newContext: () => context }), {
...
function locationsOf(...cities: string[]): RawCompanyLocation[] {
  return cities.map((city, i) => ({ isPrimary: i === 0, lines: [`${city} Street 1`, `${city}, DE`] }));
}

test('addressesFor parses the locations off the company page, primary first', async () => {
  const lookup = await makeLookup({ locationsFor: () => locationsOf('Frankfurt', 'Kassel') });

  const addresses = await lookup.addressesFor(YATTA);

  assert.deepEqual(addresses?.map((a) => a.city), ['Frankfurt', 'Kassel']);
  assert.equal(addresses?.[0]?.streetAddress, 'Frankfurt Street 1');
});

test('addressesFor clears cookies before every navigation', async () => {
  // Load-bearing, not hygiene: LinkedIn only serves the Locations section to a
  // cookie jar that has not already seen a company page. Without this the
  // second company onwards silently comes back with no addresses at all.
  const pending = makeLookup({ locationsFor: () => locationsOf('Frankfurt') });
...
test('addressesFor serves a repeated company from cache without navigating again', async () => {
  const pending = makeLookup({ locationsFor: () => locationsOf('Frankfurt') });
  const lookup = await pending;
...
test('addressesFor retries once when the page comes back with no locations section', async () => {
  // The section is served intermittently — the same company answers with
  // addresses on one load and nothing on the next.
  const pending = makeLookup({
    locationsFor: (_url, attempt) => (attempt === 0 ? [] : locationsOf('Frankfurt')),
  });
  const lookup = await pending;
  ...
  assert.deepEqual(addresses?.map((a) => a.city), ['Frankfurt']);
});

test('addressesFor gives up with an empty array when the retry is also empty', async () => {
  // `[]` and null are different answers: this company was read successfully
  // and genuinely publishes nothing.
...
test('addressesFor returns null when LinkedIn redirects to the sign-up wall', async () => {
  const lookup = await makeLookup({
    locationsFor: () => locationsOf('Frankfurt'),
    landsOn: () => 'https://www.linkedin.com/authwall?trk=bf&sessionRedirect=x',
  });

  assert.equal(await lookup.addressesFor(YATTA), null);
});

test('addressesFor returns null when the navigation throws', async () => {
  const lookup = await makeLookup({
    locationsFor: () => locationsOf('Frankfurt'),
    onGoto: () => {
...
test('addressesFor caches a failure so a broken company page is not reloaded once per job', async () => {
  const pending = makeLookup({
    locationsFor: () => locationsOf('Frankfurt'),
    onGoto: () => {
      throw new Error('net::ERR_TIMED_OUT');
...
test('addressesFor returns null without navigating when the card carried no company URL', async () => {
  const pending = makeLookup({ locationsFor: () => locationsOf('Frankfurt') });
  const lookup = await pending;
...
test('maxAddressesPerCompany caps the list while keeping the primary address', async () => {
  const lookup = await makeLookup({
